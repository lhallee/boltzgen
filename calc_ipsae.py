...
pipeline. ipSAE is a confidence metric where HIGHER scores indicate BETTER
...
2. HANDLING MULTIPLE PAE SAMPLES
   ...
   confidence estimates, and taking the best (highest) sample gives the most
...
def run_ipsae_on_single_pae(
    ...
    Returns:
        float: The ipSAE score (higher is better), or -inf on failure
    """
...
def calculate_ipsae(
    ...
    Returns:
        float: The best ipSAE score across all samples (higher is better)
    """
    # Load the npz file
    try:
...
def process_output_dir(
    ...
    # Sort by ipSAE (higher is better)
    designs.sort(key=lambda x: x['ipsae'], reverse=True)
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("ipSAE RANKING (higher is better)")
    print("=" * 60)
